...
def plot_categotical_features_accept(data, plot_wandb=False):
    ...
    for x in cat_cols:
        ...
        if len(filtered_num) > 15:
            fig = px.bar(
                x=filtered_num[:15].keys(),
                y=filtered_num[:15].values,
                color=filtered_num[:15].values,
            )
            fig.update_layout(
                title=f"{x} top 15 values", xaxis_title=x, yaxis_title="count"
            )
            fig.show()
            ...
            fig = px.bar(
                x=filtered_num.keys(), y=filtered_num.values, color=filtered_num.values
            )
            fig.update_layout(title=f"{x}", xaxis_title=x, yaxis_title="count")
            fig.show()
...
def earliest_cr_line_plot(df, plot_wandb=False):
    cf = df.copy()
    cf["earliest_cr_line"] = (
        cf["earliest_cr_line"].dropna().apply(lambda date: int(date[-4:]))
    )
...
def draw_correletion_plot(df, plot_wandb=False):
    ...
    df_temp = df.copy()
    df_temp = df_temp[
        (df_temp["loan_status"] == "Fully Paid")
        | (df_temp["loan_status"] == "Charged Off")
    ]
    df_temp["loan_status"] = pd.get_dummies(df_temp["loan_status"], drop_first=True)
    fig = plt.figure(figsize=(10, 6))
    plot = (
        df_temp.corr()["loan_status"]
        .sort_values()
        .drop("loan_status")
        .plot(kind="bar", cmap="viridis")
    )  # correlation with loan_status for continuous features with loan_status feature dropped
