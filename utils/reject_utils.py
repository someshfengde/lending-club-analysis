...
def plot_reject_yearwise(data, plot_wandb=False):
    ...
    fig.update_layout(
        title_text="rejected application year wise",
        xaxis_title="year",
        yaxis_title="count",
    )
    fig.show()
...
def plot_loan_title_category(data, plot_wandb=False):
    ...
    fig = px.bar(
        x=data_counts[:10].keys(),
        y=data_counts[:10].values,
        color=data_counts[:10].values,
    )
    fig.update_layout(
        title_text="top 10 rejected loan titles",
        xaxis_title="loan title",
        yaxis_title="count",
    )
    fig.show()
    if plot_wandb:
    ...
    fig.update_layout(
        title_text="least rejected loan titles",
        xaxis_title="loan title",
        yaxis_title="count",
    )
    fig.show()
...
def plot_state_category(data, plot_wandb=False):
    ...
    fig.update_layout(
        title_text="top 10 rejected application states",
        xaxis_title="state",
        yaxis_title="count",
    )
...
def plot_employment_length_category(data, plot_wandb=False):
    ...
    fig.update_layout(
        title_text="rejected application reletion with employment length",
...
def plot_policy_code_category(data, plot_wandb=False):
    ...
    fig = px.bar(x=data.keys(), y=data.values, color=data.values)
    fig.update_layout(
        title_text="rejected application reletion with policy code",
        xaxis_title="Policy Code",
        yaxis_title="count",
    )
    fig.show()
